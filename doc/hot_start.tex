...
\SaveVerb{modify_type_inactive}|modify_type_inactive|
\SaveVerb{modify_type_active}|modify_type_active|
\SaveVerb{x_guess_is_provided}|x_guess_is_provided|
\SaveVerb{modify_x_guess}|modify_x_guess|
...
\item {\color{blue}$\{\mathcal{W}^{\mathit{guess}}, x^{\mathit{guess}}\}$} + flags (or when
  ...
  \item[\ding{237}] {\bf IF}
    ...
    \begin{itemize}
    \item \UseVerb{modify_x_guess} = \UseVerb{false}
    \item \UseVerb{modify_type_inactive} = \UseVerb{false} 
    \item \UseVerb{modify_type_active} = \UseVerb{false}
    \end{itemize}
    %
    $\mathcal{W}^{(0)} \leftarrow \mathcal{W}^{\mathit{guess}}$. Equality constraints are detected
    internally and included in $\mathcal{W}^{(0)}$. 
  ...
  \item[\ding{237}] {\bf ELSE IF}
    ...
    \begin{itemize}
    \item \UseVerb{modify_x_guess} = \UseVerb{false}
    \item {\color{blue}\UseVerb{modify_type_inactive} = \UseVerb{true}}
    \item \UseVerb{modify_type_active} = \UseVerb{false}
    \end{itemize}
    % 
  ...
  \item[\ding{237}] {\bf ELSE IF}
    ...
    \item \UseVerb{modify_x_guess} = \UseVerb{false}
    \item \UseVerb{modify_type_inactive} = \UseVerb{false} 
    \item {\color{blue}\UseVerb{modify_type_active} = \UseVerb{true}}
    \end{itemize}
    % 
  ...
  \item[\ding{237}] {\bf ELSE IF}
    ...
    \begin{itemize}
    \item {\color{blue}\UseVerb{modify_x_guess} = \UseVerb{true}}
    \item \UseVerb{modify_type_inactive} = \UseVerb{false} 
    \item \UseVerb{modify_type_active} = \UseVerb{false}
    \end{itemize}
    %
...
\item \UseVerb{CTR_INACTIVE}
  ...
  \item ({\bf as user input}) Specifies that the $i$-th constraint should not be included in
    ...
    \item[\ding{237}] \UseVerb{modify_type_inactive} = \UseVerb{true} (see Algorithm~\ref{alg.type}).
    \end{itemize}
  \end{itemize}
...
\item \UseVerb{CTR_ACTIVE_LB}
  ...
  \item ({\bf as user input}) Specifies that the $i$-th constraint should be included in the
    ...
    \item[\ding{237}] $\ubar{b}_i = \bar{b}_i$ (the $i$-th constraint is automatically modified to
      \UseVerb{CTR_ACTIVE_EQ}).
    \item[\ding{237}] \UseVerb{modify_type_active} = \UseVerb{true} (see Algorithm~\ref{alg.type}).
    \end{itemize}
  \end{itemize}
...
\item \UseVerb{modify_type_inactive}
...
\item \UseVerb{modify_type_active}
...
\item \UseVerb{x_guess_is_provided}
  ...
  $\ubar{b}_i \neq \bar{b}_i$). Note that the flag \UseVerb{x_guess_is_provided} is not directly set
  by the user.
...
\begin{algorithm} \label{alg.type}
  ...
  \KwData{\UseVerb{type}$\in\{$\UseVerb{CTR_INACTIVE}, \UseVerb{CTR_ACTIVE_LB}, \UseVerb{CTR_ACTIVE_UB}, \UseVerb{CTR_ACTIVE_EQ}$\}$ 
    ...
    \hspace{1.4cm}\UseVerb{x_guess_is_provided}, \\
    \hspace{1.4cm}\UseVerb{modify_type_inactive}, \\
    \hspace{1.4cm}\UseVerb{modify_type_active}
  }
  \KwOut{\UseVerb{type}}
  ...
  \If{\upshape\color{blue}\UseVerb{x_guess_is_provided}}
      {
        \vspace{0.2cm}
        
        \If{\upshape{\color{blue}\UseVerb{type} = \UseVerb{CTR_INACTIVE}} {\bf and} \color{blue}\UseVerb{modify_type_inactive}}{
        ...
        \If{\upshape{\color{blue}\UseVerb{type} = \UseVerb{CTR_ACTIVE_LB}} {\bf and} \color{blue}\UseVerb{modify_type_active}}
           {
        ...
        \If{\upshape{\color{blue}\UseVerb{type} = \UseVerb{CTR_ACTIVE_UB}} {\bf and} \color{blue}\UseVerb{modify_type_active}}
           {
...
\begin{algorithm} \label{alg.x_guess}
  ...
  \KwData{\UseVerb{type}$\in\{$\UseVerb{CTR_INACTIVE}, \UseVerb{CTR_ACTIVE_LB}, \UseVerb{CTR_ACTIVE_UB}, \UseVerb{CTR_ACTIVE_EQ}$\}$ 
    ({\color{blue}from Algorithm~\ref{alg.type}}), \\
    \hspace{1.2cm} \UseVerb{modify_x_guess}}
  \KwOut{(possibly) modified $x^{\mathit{guess}}$}
  
  \vspace{0.2cm}

  \eIf{\upshape\color{blue}\UseVerb{modify_x_guess} = \UseVerb{false}}{\KwRet $x^{\mathit{guess}}$}
      {
        \vspace{0.2cm}
...
\begin{algorithm} \label{alg.W0}
  \caption{form $W^{(0)}$}
  \KwData{\UseVerb{x_guess_is_provided}, \UseVerb{modify_x_guess}}
  
  \vspace{0.2cm}

  \eIf{\upshape{\color{blue}\UseVerb{x_guess_is_provided}} {\bf and} {\color{blue}\UseVerb{modify_x_guess}} {\bf and} {\color{blue}\UseVerb{SIMPLE_BOUNDS_OBJECTIVE}}}
      {
        \For{$i \in \{$all constraints in first objective$\}$}
            {
