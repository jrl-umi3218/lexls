...
src_files=`find include interfaces tests -name '*.cpp' -or -name '*.h' -or -name '*.hpp'|grep -Ev "utils/mc_bin_flatbuffers/"|grep -Ev "include/mc_rtc/robin_hood/robin_hood.h"`
