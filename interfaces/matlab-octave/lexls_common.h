...
void checkInputMatrix(const mxArray *M, const int objective_index, const char *name)
{
    ...
    failIfTrue(M == NULL,
               (std::string("Matrix '") + name + "' is missing on level " + str_objective_index.str() + ".").c_str());
    //failIfTrue (mxIsEmpty (M), (std::string("Matrix '") + name + "' is empty on level " + str_objective_index.str() + ".").c_str());
    warnIfTrue(mxIsEmpty(M), (std::string("Matrix '") + name + "' is empty on level " + str_objective_index.str() + ".")
                                 .c_str()); // NOT PROPERLY TESTED
    failIfTrue(!mxIsDouble(M), (std::string("Matrix '") + name + "' must be of 'double' type on level "
                                + str_objective_index.str() + ".")
                                   .c_str());
}

void checkInputMatrixSize(const mxArray *M,
                          ...
                          const char *name)
{
    ...
    failIfTrue(mxGetM(M) != nrows, (std::string("Wrong number of rows in matrix '") + name + "' on level "
                                    + str_objective_index.str() + ".")
                                       .c_str());
    failIfTrue(mxGetN(M) != ncols, (std::string("Wrong number of columns in matrix '") + name + "' on level "
                                    + str_objective_index.str() + ".")
                                       .c_str());
}
 ...
 */
void checkInputOutput(const int num_output,
                      mxArray *output[],
                      const int num_input,
                      const mxArray *input[],
                      const int min_outputs,
                      const int max_outputs,
                      const int min_inputs,
...
bool getOptionBool(bool *option_value, const mxArray *option_struct, const char *option_id)
{
    ...
    if (option != NULL)
    {
        failIfTrue(!mxIsDouble(option),
                   (std::string("Flag option '") + option_id + "' must be of 'double' type.").c_str());
        if (*mxGetPr(option) != 0)
        {
            *option_value = true;
        }
...
mxArray *catenateMatrices(mxArray *matrix1, mxArray *matrix2, mxArray *matrix3 = NULL)
{
