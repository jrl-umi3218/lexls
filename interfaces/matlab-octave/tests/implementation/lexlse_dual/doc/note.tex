...
\title{Some notes on Tikhonov regularization and Lagrange multipliers} \author{Dimitar Dimitrov}
...
\begin{align} \label{eq.LS1}
  \minimize {x} \norm{\begin{bmatrix} D_1 \\ \mu_1I \end{bmatrix}x -
    \begin{bmatrix} d_1 \\ 0 \end{bmatrix}}^2,
\end{align}
%
where\footnote{Note that in the $\ell$-QR factorization we use $x_1^{\star}$ instead of $d_1$, but
  we would have to redefine $x_1^{\star}$, so I prefer to introduce $d_1$.} $d_1 = Q_1^{'T}y_1$. The
solution of~\eqref{eq.LS1}, $x^{(1)}$, can be found using the Normal equations
%
\begin{align*}
  x^{(1)} = (D_1^TD_1 + \mu_1^2I)^{-1}D_1^Td_1.
\end{align*}
%
