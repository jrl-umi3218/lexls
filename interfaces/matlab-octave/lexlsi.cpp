...
mxArray *formInfoStructure(const LexLS::TerminationStatus lexlsi_status,
                           ...
                           const int cycling_counter)
{
    ...
    const char *info_field_names[] = {
        "status", "number_of_activations", "number_of_deactivations", "number_of_factorizations", "cycling_counter",
    };
...
mxArray *formDebugStructure(const std::vector<LexLS::WorkingSetLogEntry> &working_set_log,
                            ...
                            const LexLS::dMatrixType &problem_data)
{
    ...
    const char *info_field_names[] = {"working_set_log", "active_ctr", "lambda",   "lexqr",      "data",
                                      "xStar",           "X_mu",       "X_mu_rhs", "residual_mu"};
    ...
    const char *ws_info_field_names[] = {"obj_index",       "ctr_index",        "ctr_type",
                                         "alpha_or_lambda", "cycling_detected", "rank"};
    ...
    const char *active_ctr_field_names[] = {"obj_index", "ctr_index", "ctr_type"};
...
void mexFunction(int num_output, mxArray *output[], int num_input, const mxArray *input[])
{
    ...
    if (num_input >= 2)
    {
        ...
        if ((options_struct != NULL) && (!mxIsEmpty(options_struct)))
        {
            ...
            getOptionDouble(&lexlsi_parameters.tol_linear_dependence, options_struct, "tol_linear_dependence");

            getOptionDouble(&lexlsi_parameters.tol_wrong_sign_lambda, options_struct, "tol_wrong_sign_lambda");

            getOptionDouble(&lexlsi_parameters.tol_correct_sign_lambda, options_struct, "tol_correct_sign_lambda");

            getOptionDouble(&lexlsi_parameters.tol_feasibility, options_struct, "tol_feasibility");

            getOptionUnsignedInteger(&lexlsi_parameters.max_number_of_factorizations, options_struct,
                                     "max_number_of_factorizations");

            getOptionBool(&lexlsi_parameters.cycling_handling_enabled, options_struct, "cycling_handling_enabled");

            getOptionUnsignedInteger(&lexlsi_parameters.cycling_max_counter, options_struct, "cycling_max_counter");

            getOptionDouble(&lexlsi_parameters.cycling_relax_step, options_struct, "cycling_relax_step");

            is_regularization_set = getOptionArray(regularization_factors, mxGetNumberOfElements(input[0]),
                                                   options_struct, "regularization_factors");

            getOptionBool(&is_simple_bounds_handling_enabled, options_struct, "enable_simple_bounds");

            getOptionBool(&lexlsi_parameters.use_phase1_v0, options_struct, "use_phase1_v0");

            getOptionBool(&lexlsi_parameters.set_min_init_ctr_violation, options_struct, "set_min_init_ctr_violation");

            getOptionBool(&lexlsi_parameters.modify_x_guess_enabled, options_struct, "modify_x_guess_enabled");

            getOptionBool(&lexlsi_parameters.modify_type_active_enabled, options_struct, "modify_type_active_enabled");

            getOptionBool(&lexlsi_parameters.modify_type_inactive_enabled, options_struct,
            ...
            getOptionUnsignedInteger(&lexlsi_parameters.max_number_of_CG_iterations, options_struct,
                                     "max_number_of_CG_iterations");

            getOptionDouble(&lexlsi_parameters.variable_regularization_factor, options_struct,
                            "variable_regularization_factor");

            getOptionString(lexlsi_parameters.output_file_name, options_struct, "output_file_name", 20);

            getOptionBool(&lexlsi_parameters.deactivate_first_wrong_sign, options_struct,
            ...
            if (((lexlsi_parameters.regularization_type == LexLS::REGULARIZATION_NONE) && (is_regularization_set))
    ...
    try
    {
        ...
        if (is_simple_bounds_handling_enabled)
        {
            lexlsi.setData(0, simple_bounds_indicies.data(),
                           Eigen::Map<LexLS::dMatrixType>(mxGetPr(constraints[0]), num_constr[0], 2));
        }
        for (unsigned int i = first_normal_obj_index; i < num_obj; ++i)
        {
            lexlsi.setData(i, Eigen::Map<LexLS::dMatrixType>(mxGetPr(constraints[i]), num_constr[i], num_var + 2));
        }
    ...
    try
    {
        const LexLS::dVectorType &x = lexlsi.get_x();
    ...
    if (num_output >= 5)
    {
        ...
        output[4] = formDebugStructure(working_set_log, active_ctr, lambda, lexqr, xStar, X_mu, X_mu_rhs, residual_mu,
                                       problem_data);
    }
}
