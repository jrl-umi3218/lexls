...
namespace LexLS
{
    namespace tools
    {
        ...
        private:
            ...
            inline void readNumberOfObjectives(std::ifstream &ifs,
            ...
            inline void readTypeOfHieararchy(std::ifstream &ifs,
            ...
            inline void readNumberOfConstraints(std::ifstream &ifs,
            ...
            inline void readTypesOfObjectives(std::ifstream &ifs,
                                      ...
                                      std::vector<std::vector<ConstraintActivationType>> &active_set_guess)
            {
                ...
                for (unsigned int row_index = 0; row_index < number_of_rows; ++row_index)
                {
                    ...
                    while (!line_stream.eof() && !line_stream.fail() && (column_index < number_of_columns))
                    {
        ...
        public:
                               ...
                               Eigen::VectorXd &solution)
            {
                ...
                while (getline(ifs, line)
                       && !(set_number_of_variables && set_number_of_objectives && set_type_of_hierarchy
                            && set_types_of_objectives && set_number_of_constraints))
                {
                ...
                if (!set_number_of_variables || !set_number_of_objectives || !set_type_of_hierarchy
                    || !set_types_of_objectives || !set_number_of_constraints)
                {
                ...
                while (getline(ifs, line) && (objective_index < number_of_objectives))
                {
                    ...
                    if (line.compare(0, OBJECTIVE_DATA.length(), OBJECTIVE_DATA) == 0)
                    {
                        if (types_of_objectives[objective_index] == LexLS::SIMPLE_BOUNDS_OBJECTIVE)
                        {
                            ...
                            else
                            {
                                throw std::runtime_error(
                                    "Simple constraints are supported only in the first objective.");
                            }
                        }
                        else
                        {
                        ...
                        readObjective(number_of_constraints[objective_index], number_of_columns, objective_index,
                                      type_of_hierarchy_header, ifs, objectives, active_set_guess);
